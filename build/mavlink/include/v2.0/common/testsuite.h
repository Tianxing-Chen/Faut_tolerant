...
static void mavlink_test_rotor_control(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_ROTOR_CONTROL >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_rotor_control_t packet_in = {
        17.0,{ 45.0, 46.0, 47.0, 48.0 },{ 157.0, 158.0, 159.0, 160.0 },113
    };
    mavlink_rotor_control_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.thrust = packet_in.thrust;
        packet1.control_mode = packet_in.control_mode;
        
        mav_array_memcpy(packet1.mot_throttle, packet_in.mot_throttle, sizeof(float)*4);
        mav_array_memcpy(packet1.rotor_thrust, packet_in.rotor_thrust, sizeof(float)*4);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_ROTOR_CONTROL_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_ROTOR_CONTROL_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_rotor_control_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_rotor_control_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_rotor_control_pack(system_id, component_id, &msg , packet1.control_mode , packet1.thrust , packet1.mot_throttle , packet1.rotor_thrust );
    mavlink_msg_rotor_control_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_rotor_control_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.control_mode , packet1.thrust , packet1.mot_throttle , packet1.rotor_thrust );
    mavlink_msg_rotor_control_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_rotor_control_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_rotor_control_send(MAVLINK_COMM_1 , packet1.control_mode , packet1.thrust , packet1.mot_throttle , packet1.rotor_thrust );
    mavlink_msg_rotor_control_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_rotor_control(system_id, component_id, last_msg);
}
